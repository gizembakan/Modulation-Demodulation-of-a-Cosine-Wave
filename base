...
% m(t) işaretinin periyodu, T = 0.02s olarak, m1(t) ve m2(t)
işaretlerinin
...
edilmiştir.
m_f = abs(fftshift(fft(m_t)))/N; % fftshift ile f=0 merkezli olacak
şekilde dönüşüm elde edilmiştir.
figure();
plot(f_axis, m_f);
xlabel('frekans(Hz)');
ylabel('genlik')
title('M(f) = 10*[dirac(f-50)+dirac(f+50)]+5*[dirac(f-
100)+dirac(f+100)]');
